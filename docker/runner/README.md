...
| uid/gid mapping helpers | `newuidmap`, `newgidmap` | Debian `uidmap` package, `setcap`'d (see [Notes](#notes)) |
...
| `USER` | `nonroot` | Required by buildkitd's rootless-defaults gate (see [Notes](#notes)) |
| `HOME` | `/home/nonroot` | Steers buildkitd's state dir once that gate is open; also the general-purpose `$HOME` base images normally leave unset |
...
- **`newuidmap`/`newgidmap` are `setcap`'d, not setuid.** The kernel accepts a new user namespace's `uid_map` write from a process that either holds `CAP_SYS_ADMIN`, or whose euid equals the namespace's owner (`user_namespace.c` `map_write` → `file_ns_capable`). Debian's `newuidmap` package is built without `<sys/capability.h>` (no `libcap-dev` in Debian's build chroot), so the upstream `shadow` code path that drops euid to the real uid before writing — the thing that lets it satisfy the second rule — is compiled out; as shipped it writes as euid 0 via the setuid bit and needs `CAP_SYS_ADMIN`, which no container has by default. Symptom if this regresses: `newuidmap: write to uid_map failed: Operation not permitted`, with everything else (setuid bit present, subuid range correct, `NoNewPrivs: 0`) looking fine — a genuinely confusing kernel-level EPERM, not one of `newuidmap`'s own error messages. Fix: `setcap cap_setuid+ep`/`cap_setgid+ep` plus dropping the setuid bit, matching how Alpine's `shadow-uidmap` (the vendor image's own `newuidmap`) is built in the first place. This is also why the pool no longer needs `securityContext.capabilities.add: [SYS_ADMIN]` on either the runner or job pod in `pyck-ai/deployment` — that was the workaround before this was understood; this image no longer needs it granted at all.
- **`/go/pkg` is explicitly `chown`'d.** `docker/base` and `docker/golang`'s `install -d -o nonroot -g nonroot /go /go/bin /go/pkg/mod /var/cache/go` names `/go/pkg/mod` but never its parent `/go/pkg`, which `install(1)` then creates as an implicit parent with default root ownership. Invisible until a module needs sumdb verification for the first time: Go can write existing entries under the already-nonroot `/go/pkg/mod` fine, but `mkdir`'ing a new sibling like `/go/pkg/sumdb/` hits `EPERM` on `/go/pkg` itself, surfacing as `open /go/pkg/sumdb/sum.golang.org/latest: no such file or directory` — reads like a network failure and is not one (confirmed live: DNS resolves, `sum.golang.org` and the `GOPROXY` sumdb-proxy path both return 200).
- **`USER`/`HOME` matter specifically for buildkitd.** Its rootless-defaults gate (`isRootlessConfig`, `cmd/buildkitd/main.go`) checks `$USER` is non-empty and not `"root"` before it will steer its state dir off the root-owned `/var/lib/buildkit`; `$HOME` only feeds that computation once the gate is already open (`appdefaults.UserRoot()` falls back to `/var/lib/buildkit`, which uid 1001 cannot `mkdir` into, if `HOME` is empty too). Confirmed the hard way — `HOME` set without `USER` still hits `mkdir /var/lib/buildkit: permission denied`.
- **subuid/subgid**: RootlessKit always shells out to `newuidmap`/`newgidmap` and hard-fails without a subuid range — there is no single-uid fallback. `/etc/subuid` and `/etc/subgid` carry `nonroot:100000:65536`. The vendor image preseeds the same range for its own user, which is why a build that passes in `moby/buildkit:rootless` fails in any hand-rolled image that forgets it. The vendor's `newuidmap`/`newgidmap` are musl-linked and `COPY` drops their file capabilities, so they come from Debian's `uidmap` instead (with the `setcap` fix above).
...
- **Verification**: `verify.sh` does not perform an actual rootless build. That needs unprivileged nested user namespaces, which the CI host (a privileged `dind` sidecar on Ubuntu 24.04, `kernel.apparmor_restrict_unprivileged_userns=1`) does not grant regardless of the image being correct. It checks presence, versions, subuid ranges and capabilities instead; the end-to-end build is proven on the pool itself by pyck-ai/deployment's kata smoke test — where this exact image, before this fix, needed `SYS_ADMIN`/`USER`/`HOME`/`GOPATH` patched in at the pod-spec level to work at all.

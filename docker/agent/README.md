...
Runs as **root (uid 0) by default**. A `nonroot` account (uid/gid 65532) still exists, and `/bun` is nonroot-owned, so `bun add -g` also works under `--user 65532`, landing the resulting binary on `PATH`. `WORKDIR` is `/app`.
...
```sh
docker run --rm --user 65532 ghcr.io/pyck-ai/baseimages/agent:latest claude --version
```
